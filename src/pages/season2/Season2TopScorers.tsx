import { getSeason2TopScorers } from "@/data/season2Data";
import Season2Shell, { Season2PageHeader } from "./Season2Shell";

export default function Season2TopScorers() {
  const rows = getSeason2TopScorers();

  return (
    <Season2Shell>
      <main className="min-h-screen bg-[#f7f7f2] pb-12">
        <Season2PageHeader eyebrow="Attack" title="Бомбардири" text="Рейтинг атаки Season 2. Після перших результатів тут одразу зʼявляться голи." />
            ...
            <table className="w-full min-w-[620px] text-left sm:min-w-[680px]">
              <thead className="bg-[#111111] text-[#f7f7f2]">
                <tr className="text-[0.68rem] uppercase tracking-wide sm:text-xs">
                  ...
                  <th className="px-3 py-3 text-center sm:px-4">Голів</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#111111]/10">
                {rows.map((row, index) => (
                  <tr key={row.player.id}>
                    ...
                    <td className="px-3 py-3 text-center font-heading text-xl text-[#ff5a1f] sm:px-4 sm:py-4 sm:text-2xl">{row.goalsFor}</td>
                  </tr>
                ))}
              </tbody>
            </table>
