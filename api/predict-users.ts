...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  try {
    ...
    const [users, predictions, tournaments, currentUser] = await Promise.all([
      supabaseGet<DbUser[]>("/predict_users?select=id,username,invite_code,invited_by,invites_remaining,is_admin,favorite_team,total_points,created_at&order=total_points.desc"),
      supabaseGet<Array<DbPrediction & { user_id: string }>>("/predict_predictions?select=user_id,match_id,predicted_home_score,predicted_away_score,predicted_advancing,points_outcome,points_advancing,created_at"),
