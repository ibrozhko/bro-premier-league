...
async function recalculatePredictions(
  ...
  const rows = await supabaseGet<Array<{
    ...
    `/predict_predictions?select=id,predicted_home_score,predicted_away_score,predicted_home_penalties,predicted_away_penalties,predicted_advancing,predict_matches!inner(stage,team_advancing)&predict_matches.external_id=eq.${encodeURIComponent(externalId)}`,
  );
